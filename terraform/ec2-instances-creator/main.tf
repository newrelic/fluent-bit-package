...
module "ec2_instance" {
  ...
  # Render the user data script for the SSM-install distros above, plus Ubuntu 26.04 (resolute) only - resolute ships sudo-rs,
  # whose use_pty default breaks Ansible `become` over the aws_ssm connection (see the .tftpl). All other distros/versions
  # (including Ubuntu 22.04/24.04) keep their previous behaviour (no user data).
