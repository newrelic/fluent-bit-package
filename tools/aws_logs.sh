...
if [[ "${listSize}" -gt 0 ]]; then
    result="$(aws logs describe-log-streams --region ${region} --limit ${listSize} --log-group-name ${groupName} --order-by LastEventTime --descending)"
    status=$?
...
while : ; do
    ...
    result="$(aws logs get-log-events --region ${region} ${paginationArg} --log-group-name ${groupName} --log-stream-name ${streamName} --output json)"
